/**
 * @file /src/win_bin.cpp
 *
 * @brief Self describing executable for launching python scripts in win_ros/bin
 *
 * This is waiting until we can get rosstack find working.
 *
 * @date March 2011
 **/
...
char name[256];
std::string win_ros_root, python_home, python_file, python_exe;
...
void debug() {
	...
	std::cout << "  win_ros_root: " << win_ros_root << std::endl;
...
//	}
	...
	//win_ros_root = std::getenv("win_ros_root"); need to use rosstack find here instead.
	...
	python_file = win_ros_root+std::string("/bin/")+std::string(name);
